...
// Сборка папки inner-pages
function pages() {
  return gulp.src(`${srcFloder}inner-pages/**`)
    .pipe(size())
    .pipe(gulp.dest(`${distFloder}inner-pages/`))
    .pipe(browserSync.stream())
}
...
function watch() {
  ...
  gulp.watch(`${distFloder}inner-pages/`, pages);
}

// Настройка gulp build
const build = gulp.series(clean, html, gulp.parallel(styles, scripts, images, resources, pages), watch);
